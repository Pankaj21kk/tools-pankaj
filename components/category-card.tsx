...
function CategoryPreview({ category }: { category: ToolCategory }) {
  ...
  if (category.id === "game-development") {
    return (
      <div className={`relative flex h-full min-h-36 items-center justify-center overflow-hidden rounded-2xl border-2 ${getCategoryBorder(category.id)} bg-white sm:min-h-44 dark:bg-zinc-900`}>
        <OrbitingCircles radius={44} iconSize={18} duration={18}>
  ...
  if (category.id === "video-editing" || category.id === "mechanical-engineering") {
    return (
      <div className={`rounded-2xl border-2 ${getCategoryBorder(category.id)} bg-white p-2.5 sm:p-3 dark:bg-zinc-900`}>
        ...
        <p className="mt-2 text-center text-xs font-semibold text-zinc-700 sm:mt-3 sm:text-sm dark:text-zinc-200">
          {category.visual}
        </p>
      </div>
    );
      ...
      <div className={`rounded-2xl border-2 ${getCategoryBorder(category.id)} bg-white p-2.5 sm:p-3 dark:bg-zinc-900`}>
        ...
        <div className="mt-3 flex items-end justify-between">
          <div>
            ...
            <NumberTicker value={count} className="mt-1 text-3xl font-bold text-zinc-900 sm:text-4xl dark:text-zinc-100" />
          </div>
  ...
  if (category.id === "ui-ux" || category.id === "researcher" || category.id === "civil-engineering") {
    return (
      <div className={`relative overflow-hidden rounded-2xl border-2 ${getCategoryBorder(category.id)} bg-white p-3 sm:p-4 dark:bg-zinc-900`}>
    ...
    >
      <div className={`grid gap-3 rounded-lg border-2 ${getCategoryBorder(category.id)} bg-white p-3 shadow-sm sm:gap-5 sm:p-4 md:grid-cols-[1fr_220px] md:items-center lg:grid-cols-[1fr_260px] dark:bg-zinc-900`}>
        <div className="min-w-0">
          ...
          <h2 className="mt-1.5 text-lg font-bold text-zinc-900 sm:mt-2 sm:text-2xl dark:text-zinc-100">
            {category.title}
          </h2>
